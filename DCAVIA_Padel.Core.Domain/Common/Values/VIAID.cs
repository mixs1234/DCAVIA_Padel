...
public class VIAID : ValueObject
{
    ...
    public static Result<VIAID> Create(string value)
    {
        ...
        return validation.IsFailure ? ResultBase.Fail<VIAID>(validation.Error!) : ResultBase.Ok(viaID);
    }
    
    public override Result Validate()
    {
        if (string.IsNullOrWhiteSpace(Value))
            return ResultBase.Fail(new ResultError("422", "VIA ID cannot be empty."));
        ...
        switch (isAllLetters)
        {
            case false when !isAllDigits:
                errors.Add(new ResultError("422", "VIA ID must be either only letters or only numbers."));
                break;
            case true:
            {
                if (Value.Length < 3 || Value.Length > 4)
                    errors.Add(new ResultError("422", "VIA ID with letters must be between 3 and 4 characters long."));

                if (Value.Any(char.IsUpper))
                    errors.Add(new ResultError("422", "VIA ID with letters must be lowercase only."));
                break;
            }
            default:
            {
                ...
                {
                    if (Value.Length != 6)
                        errors.Add(new ResultError("422", "VIA ID with numbers must be exactly 6 digits long."));
                }
